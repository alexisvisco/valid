...
func LessThan[T ishelper.Number](limit T) Rule {
	...
	return func(_ context.Context, value any) *Violation {
		...
		if !ok || n.Cmp(boundary) >= 0 {
			return &Violation{
				Code:    ViolationLT,
				Message: formatMessage(ViolationLT, map[string]any{"value": limit}),
			}
		}
		return nil
	}
